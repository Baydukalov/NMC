...
// +++.17/12/04-17:57:01.<УП>
	//ИначеЕсли НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
	ИначеЕсли Истина
		И НЕ Пользователи.ЭтоПолноправныйПользователь()
		И Параметры.Сценарий <> "СозданиеЗаявокНаПеревозку"
		И Параметры.Сценарий <> "упОперацияСистемыПлатон"
	Тогда
// ---.17/12/04-17:57:01.<УП>
	...
	// +++.16/12/01-11:55:46.<УП>
	ИначеЕсли Параметры.Свойство("ТипОбъекта") Тогда
		ОбъектМетаданныхДляВывода = Метаданные[Параметры.ВидОбъекта][Параметры.ТипОбъекта];
		...
		СписокВариантовЗагрузки.Добавить(ИнформацияОТипеЗагрузки, ОбъектМетаданныхДляВывода.Представление());
	// ---.16/12/01-11:55:46.<УП>
	Иначе
...
Процедура ПереходКСледующемуШагуЗагрузкиДанных()
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ВыборСправочникаДляЗагрузки Тогда
		// +++.16/12/01-12:00:02.<УП>
		//ОписаниеСтрокиВыбора = Элементы.ВидЗагрузкиДанных.ТекущиеДанные.Значение;
		//
		Если СписокВариантовЗагрузки.Количество() = 1 Тогда
			ОписаниеСтрокиВыбора = СписокВариантовЗагрузки.Получить(0).Значение;
		Иначе		
			ОписаниеСтрокиВыбора = Элементы.ВидЗагрузкиДанных.ТекущиеДанные.Значение;
		КонецЕсли;
		// ---.16/12/01-12:00:02.<УП>
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных Тогда
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			АдресЗагруженныхДанных = АдресВХранилищеТаблицыСопоставления();
			Закрыть(АдресЗагруженныхДанных);
		// +++.17/12/27-15:38:27.<УП>
		ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда	
			АдресЗагруженныхДанных = АдресВХранилищеТаблицыСопоставленияДанных();
			Закрыть(АдресЗагруженныхДанных);
		// ---.17/12/27-15:38:27.<УП>
...
// +++.17/12/27-15:38:10.<УП>
&НаСервере
Функция АдресВХранилищеТаблицыСопоставленияДанных()
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	// +++.16/12/14-12:05:05.<УП> https://partners.v8.1c.ru/forum/t/1556151/m/1556151
	Если Ложь Тогда // Для контекстной подсказки
	    ИнформацияПоКолонкамТаблица = Новый ТаблицаЗначений;
	КонецЕсли;
	РеквизитыТаблицы = ПолучитьРеквизиты("ИнформацияПоКолонкам");
	Для Каждого РеквизитТаблицы Из РеквизитыТаблицы Цикл
		Если ИнформацияПоКолонкамТаблица.Колонки.Найти(РеквизитТаблицы.Имя) = Неопределено Тогда
			ИнформацияПоКолонкамТаблица.Колонки.Добавить(РеквизитТаблицы.Имя, РеквизитТаблицы.ТипЗначения);
		КонецЕсли; 
	КонецЦикла;
	// ---.16/12/14-12:05:05.<УП>
...
Функция НезаполненныеОбязательныеКолонки()
	...
	Для НомерКолонки = 1 По Шапка.ШиринаТаблицы Цикл 
		...
		Если ИнформацияОКолонке <> Неопределено Тогда
			Если Истина
				И ИнформацияОКолонке.ОбязательнаДляЗаполнения 
				// @@@.16/12/01-12:02:28.<УП>
				И НЕ ЗначениеЗаполнено(ИнформацияОКолонке.ЗначениеПоУмолчанию)
			Тогда 
...
Процедура ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления, ДобавитьИдентификатор,
	...
	Если ЗначениеЗаполнено(СтруктураОбъектаСопоставления) Тогда 
		...
		// +++.16/12/01-12:03:35.<УП>
		Если Истина
			И ПараметрыЗагрузки <> Неопределено
			И ПараметрыЗагрузки.Свойство("ДобавитьПолеРеквизитов")
		Тогда 
			ВременнаяТЗ.Колонки.Добавить("ТекущиеЗначенияСтроки", Новый ОписаниеТипов("СписокЗначений"), "ТекущиеЗначенияСтроки");
			ВременнаяТЗ.Колонки.Добавить("ВидАдреса_", Новый ОписаниеТипов("Строка"), "ВидАдреса_");
		КонецЕсли;
		// ---.16/12/01-12:03:35.<УП>
	КонецЕсли;
...
Процедура ДобавитьСтандартныеКолонкиВМассивРеквизитов(МассивРеквизитов, СтруктураОбъектаСопоставления , ДобавитьИдентификатор, 
		...
		Если ЗначениеЗаполнено(СтруктураОбъектаСопоставления) Тогда 
			...
			// +++.16/12/01-12:04:13.<УП>
			Если Истина
				И ПараметрыЗагрузки <> Неопределено
				И ПараметрыЗагрузки.Свойство("ДобавитьПолеРеквизитов") 
			Тогда 
				МассивРеквизитов.Добавить(Новый РеквизитФормы("ТекущиеЗначенияСтроки", Новый ОписаниеТипов("СписокЗначений"), "ТаблицаСопоставленияДанных", "ТекущиеЗначенияСтроки"));
				МассивРеквизитов.Добавить(Новый РеквизитФормы("ВидАдреса_", Новый ОписаниеТипов("Строка"), "ТаблицаСопоставленияДанных", "ВидАдреса_"));
			КонецЕсли;
			// ---.16/12/01-12:04:13.<УП>
		КонецЕсли;
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонках()
	...
	Для Каждого Колонка Из ВременнаяТЗ.Колонки Цикл
		...
		ИначеЕсли Колонка.Имя = "РезультатСопоставленияСтроки" ИЛИ Колонка.Имя = "ОписаниеОшибки" ИЛИ Колонка.Имя = "СписокНеоднозначностей" Тогда
			НовыйЭлемент.Видимость = Ложь;
		// +++.16/12/01-11:50:30.<УП>
		ИначеЕсли Колонка.Имя = "ТекущиеЗначенияСтроки" ИЛИ Колонка.Имя = "ВидАдреса_" Тогда 
			НовыйЭлемент.Видимость = Ложь;
		// ---.16/12/01-11:50:30.<УП> 
		КонецЕсли;
