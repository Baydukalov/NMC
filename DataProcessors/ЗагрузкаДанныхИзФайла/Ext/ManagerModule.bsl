...
Процедура ЗаполнитьТаблицуСопоставленияЗагружаемымиДанными(ШаблонСДанными, ТаблицаИнформацияПоКолонкам, ТаблицаСопоставления, ФоновоеЗадание = Ложь)
	...
	// +++.16/11/29-09:54:11.<УП>
	ДобавитьПолеРеквизитов = Истина;
	Если ТаблицаСопоставления.Колонки.Найти("ТекущиеЗначенияСтроки") = Неопределено Тогда
		ДобавитьПолеРеквизитов = Ложь;
	КонецЕсли;
	// ---.16/11/29-09:54:11.<УП>
	...
	Для НомерСтроки = ПерваяСтрокаТаблицы По ШаблонСДанными.ВысотаТаблицы Цикл 
		...
		Для НомерКолонки = 1 По ШаблонСДанными.ШиринаТаблицы Цикл
			...
			Если Колонка <> Неопределено Тогда
				...
				КонецЕсли;
				// +++.16/11/29-09:54:42.<УП>
				Если ДобавитьПолеРеквизитов Тогда
					ТекущаяСтруктура = Новый Структура("Имя,Значение",Колонка.ИмяКолонки,Ячейка.Текст);
					НоваяСтрока.ТекущиеЗначенияСтроки.Добавить(ТекущаяСтруктура,Колонка.ИмяКолонки);
					Если Колонка.ИмяКолонки = "ВидАдреса" Тогда
						НоваяСтрока.ВидАдреса_= Ячейка.Текст;
					КонецЕсли;
				КонецЕсли;
				// ---.16/11/29-09:54:42.<УП>
...
Функция ЗначениеЯчейки(Колонка, ЗначениеЯчейки)
	...
	ДанныеЯчейки = "";
	// +++.16/11/29-09:55:16.<УП>
	Если ЗначениеЗаполнено(Колонка.ЗначениеПоУмолчанию) Тогда
		ДанныеЯчейки = Колонка.ЗначениеПоУмолчанию;
	...
	Для каждого ТипДанных Из Колонка.ТипКолонки.Типы() Цикл 
		...
		Если ОписаниеОбъекта.ТипОбъекта = "Справочник" Тогда
			...
			Если НЕ ЗначениеЗаполнено(ДанныеЯчейки) Тогда 
				ДанныеЯчейки = Справочники[ОписаниеОбъекта.НазваниеОбъекта].НайтиПоКоду(ЗначениеЯчейки, Истина);
			КонецЕсли;
			// +++.16/11/29-09:56:04.<УП>
			#Если Сервер Или ТолстыйКлиентОбычноеПриложение Тогда
			Если НЕ ЗначениеЗаполнено(ДанныеЯчейки) И ОписаниеОбъекта.НазваниеОбъекта = "упАдреса" Тогда 
				Координаты = упГеокодированиеКлиентСервер.НайтиКоординатыАдреса(ЗначениеЯчейки);
				Если Координаты <> Неопределено И (Координаты.Широта <> 0 ИЛИ Координаты.Долгота <> 0) Тогда 
					Запрос = Новый Запрос;
					...
					|ГДЕ
					|	упАдреса.Долгота = &Долгота
					|	И упАдреса.Широта = &Широта";
					Запрос.УстановитьПараметр("Долгота", Координаты.Долгота);
					Запрос.УстановитьПараметр("Широта", Координаты.Широта);
					
					Выборка = Запрос.Выполнить().Выбрать();
					Если Выборка.Следующий() Тогда 
						ДанныеЯчейки = Выборка.Ссылка;
					КонецЕсли;
				КонецЕсли;	
			КонецЕсли;
			#КонецЕсли
...
// +++.16/11/29-10:15:59.<УП>
// Обертка для неэкспортной функции поставщика
Функция упШиринаКолонкиПоТипу(Тип) Экспорт 
	
	Возврат ШиринаКолонкиПоТипу(Тип);
...
Функция  ПриведениеЗначениеКТипу(Значение, ОписаниеТипов)
	Для каждого Тип Из ОписаниеТипов.Типы() Цикл
		Если Тип = Тип("Дата") Тогда
			// +++.17/07/13-11:20:59.<УП>
			//изменил вызов
			Возврат упСервисныеФункцииКлиентСервер.СтрокаВДату(Значение);
...
Процедура ЗаполнитьТаблицуПоЗагруженнымДаннымИзФайла(ДанныеИзФайла, ШаблонСДанными, ИнформацияПоКолонкам)
	...
	// @@@.16/11/29-09:47:13.<УП>
	ЕстьИменаКолонок = Истина;
	
	Для каждого Колонка Из ДанныеИзФайла.Колонки Цикл
		
		// +++.16/11/29-09:47:40.<УП>
		НайденнаяКолонка = Неопределено;
		Если ЗначениеЗаполнено(СтрокаЗаголовок[Колонка.Имя]) Тогда
		// ---.16/11/29-09:47:40.<УП>
			...
			КонецЕсли;
		// +++.16/11/29-09:49:53.<УП>
		КонецЕсли;
		// Если в таблице не заданы имена колонок или имена колонок не соответствуют шаблону
		Если НайденнаяКолонка = Неопределено Тогда
			НайденнаяКолонка = ИнформацияПоКолонкам[ДанныеИзФайла.Колонки.Индекс(Колонка)];
			ЕстьИменаКолонок = Ложь;
		КонецЕсли;
		// ---.16/11/29-09:49:53.<УП>
	...
	// +++.16/11/29-09:52:19.<УП>
	//Для Индекс= 1 По ДанныеИзФайла.Количество() - 1 Цикл
	ИндексПервойСтроки = 1;
	Если НЕ ЕстьИменаКолонок Тогда
		ИндексПервойСтроки = 0;
	КонецЕсли;
	Для Индекс = ИндексПервойСтроки  По ДанныеИзФайла.Количество() - 1 Цикл
	// ---.16/11/29-09:52:19.<УП>
...
Процедура ЗагрузитьCSVФайлВТаблицу(ИмяФайла, ШаблонСДанными, ИнформацияПоКолонкам)
	...
	// +++.16/11/29-09:29:08.<УП>
	// Есть необходимость включать пустые колонки, в этом случае колонки файла и макета ставятся в соответствие посредством
	// порядкового номера.
	//КолонкиШапки = СтрРазделить(Строка, ";", Ложь);
	КолонкиШапки = СтрРазделить(Строка, ";"); 
	// ---.16/11/29-09:29:08.<УП>
	...
	Для каждого Колонка Из КолонкиШапки Цикл
		...
		// ---.16/11/29-09:39:37.<УП>
			...
			КонецЕсли;
		// +++.16/11/29-09:40:05.<УП>
		КонецЕсли; 
		// Если в таблице не заданы имена колонок или имена колонок не соответствуют шаблону.
		Если НайденнаяКолонка = Неопределено Тогда
			Если ИнформацияПоКолонкам.Количество() >= Позиция Тогда
				НайденнаяКолонка = ИнформацияПоКолонкам[Позиция - 1];
				Колонка			 = НайденнаяКолонка.ПредставлениеКолонки
			КонецЕсли;
		КонецЕсли;
		// ---.16/11/29-09:40:05.<УП>
...
// @@@.16/11/30-16:10:34.<УП>
Функция ВыделитьИмяКолонкиИзАдреса(Адрес)

	текРезультат	= "";
	МассивБукв 		= ПолучитьМассивБукв();
	текСчетчик 		= 1;
	Пока текСчетчик <= СтрДлина(Адрес) Цикл
		текСимвол = Сред(Адрес, текСчетчик, 1);
		Если НЕ МассивБукв.Найти(ВРег(текСимвол)) = Неопределено Тогда
...
КонецФункции // ВыделитьИмяКолонки()

// @@@.16/11/30-16:10:34.<УП>
Функция ПолучитьНомерКолонкиПоИмени(ИмяКолонки)
	текРезультат	= 0;
	МассивБукв 		= ПолучитьМассивБукв();
	текПорядок = 0;
	Если СтрДлина(ИмяКолонки) > 0  Тогда
		Пока Истина Цикл
			текСимвол = Прав(ИмяКолонки, 1);
			
			текНомерСимвола = МассивБукв.Найти(ВРЕГ(текСимвол));
			
			Если текНомерСимвола = Неопределено Тогда
				Прервать;
			Иначе
				текНомерСимвола = текНомерСимвола + 1;
			КонецЕсли;
			текРезультат = текРезультат + Pow(МассивБукв.Количество(), текПорядок) *  текНомерСимвола;
			
			Если СтрДлина(ИмяКолонки) > 1 Тогда
				ИмяКолонки = Лев(ИмяКолонки, СтрДлина(ИмяКолонки) - 1);
...
КонецФункции // ПолучитьНомерКолонкиПоИмени()

// @@@.16/11/30-16:10:34.<УП>
Функция ПолучитьКоличествоКолонок(Диапазон)
	
	текРезультат	= 0;
	МассивБукв 		= ПолучитьМассивБукв();
	текПозицияДвоеточия = Найти(Диапазон, ":");
	Если текПозицияДвоеточия > 0 Тогда
		текАдрес 		= Прав(Диапазон, СтрДлина(Диапазон) - текПозицияДвоеточия);
		текИмяКолонки 	= ВыделитьИмяКолонкиИзАдреса(текАдрес);
		текРезультат 	= ПолучитьНомерКолонкиПоИмени(текИмяКолонки);
	КонецЕсли;
...
// @@@.16/11/30-16:13:06.<УП>
// Перенесоно из старого БСП из-за обращения в добавленных нами методах
