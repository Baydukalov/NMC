...
&НаКлиенте
Процедура СписокВидыГСМВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ВыбраноЗначение();
КонецПроцедуры
...
Процедура ВыбраноЗначение()
	ТекущиеДанные = Элементы.СписокВидыГСМ.ТекущиеДанные;
	Если НЕ ТекущиеДанные = Неопределено Тогда
		ОповеститьОВыборе(ТекущиеДанные.Ссылка)
	Иначе
		ОповеститьОВыборе(ПредопределенноеЗначение("Справочник.упВидыГСМ.ПустаяСсылка"));
	КонецЕсли;
