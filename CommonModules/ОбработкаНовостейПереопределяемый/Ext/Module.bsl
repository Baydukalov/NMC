...
Процедура ДополнительноОбработатьТекстНовостиПередПоказом(ОбъектНовости, ТекстХТМЛ) Экспорт
	...
	// Для удобства создания новостей, можно в каждой новости вместо длинного текста вставлять некий комментарий,
	//  а перед показом "разворачивать" его в длинный текст.

	ТекстХТМЛ = СтрЗаменить(
		ТекстХТМЛ,
		"<!-- Информация о контекстной новости -->", // Идентификатор
		НСтр("ru='
			|<br/><br/>
			|<i>Комментарий к демо базе:
			|<br/>Это пример контекстной новости, привязанной к организации.
			|</i>'"))
			+ Символы.ПС;

	// Пример: Пользовательское скрытие ленты новостей.
	ТекстХТМЛ = СтрЗаменить(
		ТекстХТМЛ,
		"<!-- Скрыть эту ленту новостей -->", // Идентификатор
		НСтр("ru='
			|<br/><br/>
			|<a href=""1C:HideThisChannel"">Скрыть эту ленту новостей</a>'"))
			+ Символы.ПС;

	// Пример: замена или скрытие блока "! Важно" или "! Очень важно"
	НачалоБлокаВажности = "<a id=""importanceCaption"">";
	КонецБлокаВажности = "</a>";
	ГдеНачалоБлокаВажности = СтрНайти(ТекстХТМЛ, НачалоБлокаВажности, , 1);
	Если ГдеНачалоБлокаВажности > 0 Тогда
		ГдеКонецБлокаВажности = СтрНайти(ТекстХТМЛ, КонецБлокаВажности, , ГдеНачалоБлокаВажности);
		Если ГдеКонецБлокаВажности > 0 Тогда
			ТекстДляЗамены = НСтр("ru='<i>(важно)</i>'"); // Произвольный текст замены или пустая строка.
			ТекстХТМЛ =
				Лев(ТекстХТМЛ, ГдеНачалоБлокаВажности + СтрДлина(НачалоБлокаВажности) - 1)
				+ ТекстДляЗамены
				+ Прав(ТекстХТМЛ, СтрДлина(ТекстХТМЛ) - ГдеКонецБлокаВажности + 1);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнительноПодготовитьПараметрыНавигационнойСсылки(
	...
	Если ВРег(УИНДействия) = ВРег("1c:HideThisChannel") Тогда
		Действие = "Запуск процедуры с параметрами"; // Идентификатор.
		СписокПараметров = Новый СписокЗначений;
		СписокПараметров.Добавить("СкрытьЛентуНовостей", "Действие");
		СписокПараметров.Добавить(Объект.ЛентаНовостей, "ЛентаНовостей");
	КонецЕсли;
...
Процедура ПолучитьДополнительныеСтандартныеЗначенияКлассификаторов(ИмяОбъектаМетаданных, СтандартныеЗначения) Экспорт
	...
	// _Демо начало примера
	// Дополнительные макеты могут храниться в произвольном месте конфигурации, или даже быть переданы в виде строки.
	// В данном случае они все вставлены в отдельный справочник.
