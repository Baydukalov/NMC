...
Функция ПолучитьWSПрокси(адрес)
	...
	Попытка
		...
		//результатВозврат =  WSСсылки.ESBAdapter.СоздатьWSПрокси("http://esb.axelot.ru", soapСервис.Имя, soapСервис.ТочкиПодключения[0].Имя,,10,,адрес);
		
		Если ПолучитьФункциональнуюОпцию("упТипОбмена") = Перечисления.упТипОбмена.Web Тогда
			пользователь = сшпФункциональныеОпции.ПользовательСервиса();
			пароль = сшпФункциональныеОпции.ПарольСервиса();
			Определение = новый WSОпределения(адрес + "?wsdl", пользователь, пароль);
			
			soapСервис = Определение.Сервисы[0];
			результатВозврат = Новый WSПрокси(Определение, soapСервис.URIПространстваИмен, soapСервис.Имя, soapСервис.ТочкиПодключения[0].Имя);
			результатВозврат.Пользователь = пользователь;
			результатВозврат.Пароль       = пароль; 
		Иначе		
			soapСервис = WSСсылки.ESBAdapter.ПолучитьWSОпределения().Сервисы[0];
			результатВозврат =  WSСсылки.ESBAdapter.СоздатьWSПрокси("http://esb.axelot.ru", soapСервис.Имя, soapСервис.ТочкиПодключения[0].Имя,,120,,адрес);
		КонецЕсли;
		// ---.17/06/05-11:25:00.<УП>
	Исключение
...
Функция ПолучитьHTTPСоединение(адрес)
	...
	Если ПолучитьФункциональнуюОпцию("упТипОбмена") = Перечисления.упТипОбмена.Web Тогда
		...
		//Возврат Новый HTTPСоединение(адрес, сшпФункциональныеОпции.ПользовательСервиса(), сшпФункциональныеОпции.ПарольСервиса());
		ИмяПользователя = сшпФункциональныеОпции.ПользовательСервиса();
		ПарольПользователя = сшпФункциональныеОпции.ПарольСервиса();		
		Прокси = Новый ИнтернетПрокси(Истина);
		Соединение = Новый HTTPСоединение(адрес, , ИмяПользователя, ПарольПользователя, Прокси);
		Возврат Соединение;
		// ---.17/08/22-10:45:00.<УП>
	Иначе
		Возврат Новый HTTPСоединение(адрес);
