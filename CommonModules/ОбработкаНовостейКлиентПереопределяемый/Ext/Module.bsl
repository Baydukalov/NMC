	...
	Если ТипЗнч(СписокПараметров) = ТипСписокЗначений Тогда
		НайденноеДействие = ИнтернетПоддержкаПользователейКлиентСервер.НайтиЭлементСпискаЗначений(
			...
			Новый Структура("ВариантПоиска, ЗначениеПоиска",
				...
				"Действие"));
		Если НайденноеДействие <> Неопределено Тогда
			Если НайденноеДействие.Значение = "СкрытьЛентуНовостей" Тогда
				ЛентыНовостей = Новый Массив;
				Для Каждого ТекущийЭлементСписка Из СписокПараметров Цикл
					Если ТекущийЭлементСписка.Представление = "ЛентаНовостей" Тогда
						ЛентыНовостей.Добавить(ТекущийЭлементСписка.Значение);
					КонецЕсли;
				КонецЦикла;
				Если ЛентыНовостей.Количество() > 0 Тогда
					ОбработкаНовостейВызовСервера.ИзменитьПользовательскуюВидимостьЛентНовостей(ЛентыНовостей, Ложь);
					Оповестить("Новости. Загружены новости"); // Для обновления формы новостей, если она открыта.
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПереопределитьПараметрыОткрытияФормыКонтекстныхНовостей(ПараметрыОткрытия) Экспорт
	...
	// _Демо начало примера
	// Переопределим внешний вид формы контекстных новостей, открываемой по команде "Команда_Новость_Список":
	// - удалим колонку "Подзаголовок";
	// - покажем панель навигации.
...
Процедура КонтекстныеНовости_ОбработкаКомандыНовости(Форма, Команда) Экспорт
	...
	// _Демо начало примера
	// Во всех документах / справочниках для показа контекстных новостей используется кнопка "Новости".
	// Вместо имени команды "Команда_Новость_Список" можно использовать другие, нестандартные команды
	//  и тогда обработка будет осуществляться в этой процедуре.
	Если Команда.Имя = "ПоказатьКонтекстныеНовости"
		...
		ПараметрыОткрытияФормыКонтекстныхНовостей.Вставить("СкрыватьКолонкуПодзаголовок", Истина);
		ПараметрыОткрытияФормыКонтекстныхНовостей.Вставить("СкрыватьКолонкуДатаПубликации", Ложь);
		ПараметрыОткрытияФормыКонтекстныхНовостей.Вставить("ПоказыватьПанельНавигации", Истина);
