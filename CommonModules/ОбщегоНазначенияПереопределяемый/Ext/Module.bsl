...
//
// Параметры:
//  Обработчики - Соответствие - имена параметров сеанса и обработчики, их инициализирующие.
//
...
//
// Обработчик УстановкаПараметровСеанса имеет параметры:
//  ИмяПараметра           - Строка - имя параметра сеанса для инициализации.
//  УстановленныеПараметры - Массив - имена параметров сеанса, которые уже инициализированы.
// 
// Пример обработчика:
//
...
//   Указанные объекты и реквизиты исключаются из результатов поиска ссылок
//   и не учитываются при монопольном удалении помеченных, замене ссылок и в отчете по местам использования.
//   См. также ОбщегоНазначения.ИсключенияПоискаСсылок.
//
// Параметры:
//   ИсключенияПоискаСсылок - Массив - Объекты метаданных или их реквизиты,
//       содержимое которых не должно учитывается в бизнес-логике приложения.
//       * ОбъектМетаданных - Объект метаданных или его реквизит.
//       * Строка - Полное имя объекта метаданных или его реквизита.
//       Стандартные реквизиты допустимо указывать только в виде строк (см. пример использования № 3).
//
// Пример задачи:
//   К документу "Реализация товаров и услуг" подключены подсистемы "Версионирование объектов" и "Свойства".
...
//
// Важно:
//   Для избежания появления появления ссылок на несуществующие объекты
...
//   1. ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.ВерсииОбъектов);
//   2. ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.ВерсииОбъектов.Реквизиты.АвторВерсии);
//   3. ИсключенияПоискаСсылок.Добавить("ПланВидовРасчета._ДемоОсновныеНачисления.СтандартнаяТабличнаяЧасть.БазовыеВидыРасчета.СтандартныйРеквизит.ВидРасчета");
//
Процедура ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок) Экспорт
	
	// Управление перевозками 3.0
	
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.упРасстоянияМеждуАдресами);
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.упРасстоянияМеждуЗонами);
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.упДоступныеГеографическиеЗоны.Измерения.ГеографическаяЗона);
	
	// типы событий мониторинга
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.упДатыСобытий);
	//ИсключенияПоискаСсылок.Добавить(Метаданные.Документы.упСобытиеМониторинга.Реквизиты.ТипСобытия);
	
	// Конец Управление перевозками 3.0
...
//
// Параметры:
//   ОтключенныеПодсистемы - Соответствие - Если подсистема отключена, то метод ОбщегоНазначения.ПодсистемаСуществует
//                                          вернет Ложь.
//                                          В соответствии ключ - имя отключенной подсистемы, значение необходимо
//                                          установить в Истина.
//
