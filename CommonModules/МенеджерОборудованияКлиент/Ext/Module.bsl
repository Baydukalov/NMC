...
Процедура НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Иначе
		...
		Если ТребуетсяПроверкаАктивностиСмены Тогда
			СменаАктивна = МенеджерОборудованияВызовСервера.ВыполнитьПроверкуАктивностиСмены(ИдентификаторУстройства);
			Если Не СменаАктивна = Истина Тогда
...
Функция СформироватьТексНефискальногоЧека(ШиринаСтроки, ТипДокумента, ВходныеПараметры, ДополнительныйТекст = Неопределено) Экспорт
	...
	Если ПечатьКлише Тогда
		...
		Если ОбщиеПараметры.Свойство("АдресРасчетов") Тогда
			...
			Если НЕ ПустаяСтрока(АдресРасчетов) Тогда
				Текст = Текст + ВыстроитьПоля(АдресРасчетов, , ШиринаСтроки) + Символы.ПС;
				Если ОбщиеПараметры.Свойство("МестоРасчетов") И НЕ ПустаяСтрока(ОбщиеПараметры.МестоРасчетов) Тогда
					Текст = Текст + ВыстроитьПоля(ОбщиеПараметры.МестоРасчетов, , ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
			ИначеЕсли ОбщиеПараметры.Свойство("АдресМагазина") И НЕ ПустаяСтрока(ОбщиеПараметры.АдресМагазина) Тогда
				Текст = Текст + ВыстроитьПоля(ОбщиеПараметры.АдресМагазина, , ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
		КонецЕсли;
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Если СуммаНаличнойОплаты > 0 Тогда
			Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='НАЛИЧНЫМИ'"), "=" + Формат(СуммаНаличнойОплаты, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты1 > 0 Тогда
			Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПЛАТ.КАРТОЙ'"), "=" + Формат(СуммаБезналичнойОплаты1, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты2 > 0 Тогда
			Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='КРЕДИТОМ'"), "=" + Формат(СуммаБезналичнойОплаты2, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты3 > 0 Тогда
			Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='СЕРТИФИКАТОМ'"), "=" + Формат(СуммаБезналичнойОплаты3, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;          
		...
		Если ОбщиеПараметры.Свойство("КодСистемыНалогообложения") Тогда
			СистемаНалогообложения = МенеджерОборудованияКлиентСервер.ПолучитьНаименованиеСистемыНалогообложения(ОбщиеПараметры.КодСистемыНалогообложения, Истина);
			Текст = Текст + ВыстроитьПоля(НСтр("ru='ОСН:'"), СистемаНалогообложения, ШиринаСтроки) + Символы.ПС;
		КонецЕсли;          
