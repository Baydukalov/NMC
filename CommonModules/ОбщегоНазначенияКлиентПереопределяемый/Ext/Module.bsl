...
Процедура ПриУстановкеЗаголовкаКлиентскогоПриложения(ЗаголовокПриложения, ПриЗапуске) Экспорт
	...
	Если ПриЗапуске Тогда
		ЧасовойПоясПользователя = упСервисныеФункцииВызовСервера.НаименованиеЧасовогоПоясаТекущегоПользователя();
		
	Иначе
		ЧасовойПоясПользователя = упСервисныеФункцииВызовСервера.НаименованиеЧасовогоПоясаТекущегоСеанса();
		
	КонецЕсли;
		
	Если ЗначениеЗаполнено(ЧасовойПоясПользователя) Тогда
		ЗаголовокПриложения = ЗаголовокПриложения + " / " + ЧасовойПоясПользователя;	
	КонецЕсли;
