...
// Запишим в регистр сведений упИдентификаторыОбмена запись объекта и его идентификатро
//
// Параметры:
...
Процедура ЗаписатьВИдентификаторОбмена(Ссылка, Идентификатор = Неопределено, ИдентификаторСтрока = "", Отказ = Ложь) Экспорт
	...
	Если Ссылка <> Неопределено И ЗначениеЗаполнено(Ссылка)Тогда		
		Попытка
			Запись = РегистрыСведений.упИдентификаторыОбмена.СоздатьМенеджерЗаписи();
		...
		Исключение
			Отказ = Истина;
			ЗаписьЖурналаРегистрации(СобытиеОбменПоESB(), УровеньЖурналаРегистрации.Ошибка, , ,СтрШаблон(НСтр("ru = 'Ошибка записи в РегистрыСведений.упИдентификаторыОбмена: %1'"), ОписаниеОшибки()));
		КонецПопытки;		
...
Функция ПолучитьОбъектыПоИдентификаторам(Идентификаторы) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	упИдентификаторыОбменаПоискОбъекта.Объект,
	|	упИдентификаторыОбменаПоискОбъекта.Идентификатор
	|ИЗ
	|	РегистрСведений.упИдентификаторыОбмена КАК упИдентификаторыОбменаПоискОбъекта
	|ГДЕ
	|	упИдентификаторыОбменаПоискОбъекта.Идентификатор В(&Идентификаторы)
	|	И ТИПЗНАЧЕНИЯ(упИдентификаторыОбменаПоискОбъекта.Объект) <> ТИП(СТРОКА)";
...
Функция ПолучитьИдентификаторыПоОбъектам(Объекты) Экспорт
	...
	|ИЗ
	|	РегистрСведений.упИдентификаторыОбмена КАК усИдентификаторыОбменаПоискОбъекта
	|ГДЕ
	|	усИдентификаторыОбменаПоискОбъекта.Объект В(&Объекты)
...
Функция ПолучитьДокументПоИдентификатору(Идентификатор, Условие = "",  сткПараметры = Неопределено) Экспорт
	...
	|	РегистрСведений.упИдентификаторыОбмена КАК упИдентификаторыОбменаПоискОбъекта
	|ГДЕ
	|	упИдентификаторыОбменаПоискОбъекта.Идентификатор = &Идентификатор
	|	И ТИПЗНАЧЕНИЯ(упИдентификаторыОбменаПоискОбъекта.Объект) <> ТИП(СТРОКА) " + Условие;
...
#Область Справочник_ВидыКонтактнойИнформации
		...
		//Адреса
		МенеджерЗаписей = РегистрыСведений.упИдентификаторыОбмена.СоздатьМенеджерЗаписи();
