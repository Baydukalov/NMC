...
// +++.17/01/16-16:20:56.<УП>
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упЗаявкаНаПеревозкуГруза"));
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упЗаданиеНаПеревозкуГруза"));
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упРейс"));
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упЗаявкаНаТС"));
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упПутевойЛист"));
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упВходящаяПретензия"));
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упИсходящаяПретензия"));
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упЗаявкаНаРемонтИТОТС"));
	ТипыСоСтатусами.Добавить(Тип("ДокументСсылка.упРемонтИТОТС"));
// ---.17/01/16-16:20:56.<УП>
...
Процедура ВывестиПредставлениеИКартинку(СтрокаДерева,Макет, ЭтоТекущийОбъект = Ложь, ЭтоПодчиненный = Неопределено)
	...
	//КонецЕсли;
	//
	ТипЗначенияСсылки	= ТипЗнч(СтрокаДерева.Ссылка);
	Если НЕ ТипыСоСтатусами.НайтиПоЗначению(ТипЗначенияСсылки) = Неопределено Тогда
		Если ТипЗначенияСсылки = Тип("ДокументСсылка.упЗаявкаНаПеревозкуГруза") Тогда
			Если СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.НеЗагружена Тогда
				ИмяОбласти	= "СтатусНеЗагружена";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.Новая Тогда	
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.ПодобранаЦепьПоставок Тогда	
				ИмяОбласти	= "СтатусПодобранаЦепьПоставок";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.НеСогласована Тогда					
				ИмяОбласти	= "СтатусНеСогласовано";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.Согласована Тогда						
				ИмяОбласти	= "СтатусСогласовано";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.КВыполнению Тогда						
				ИмяОбласти	= "СтатусКВыполнению";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.СформированыЗадания Тогда						
				ИмяОбласти	= "СтатусСформированоЗадание";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.ВключенаВРейс Тогда						
				ИмяОбласти	= "СтатусВыполняется";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.Выполняется Тогда						
				ИмяОбласти	= "СтатусВПути";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.Выполнена Тогда						
				ИмяОбласти	= "ДокументПроведен";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.ВыполненаЧастично Тогда						
				ИмяОбласти	= "СтатусВыполненоЧастично";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.Отменена Тогда							
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.Отказ Тогда							
				ИмяОбласти	= "СтатусОтказ";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаПеревозкуГруза.ВПути Тогда								
				ИмяОбласти	= "СтатусВключеноВРейс";
			КонецЕсли;
		ИначеЕсли ТипЗначенияСсылки = Тип("ДокументСсылка.упЗаданиеНаПеревозкуГруза") Тогда
			Если СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.Новое Тогда
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.КПланированию Тогда
				ИмяОбласти	= "СтатусКВыполнению";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.ВключеноВРейс Тогда
				ИмяОбласти	= "СтатусВключеноВРейс";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.КВыполнению Тогда
				ИмяОбласти	= "СтатусСформированоЗадание";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.Выполняется Тогда
				ИмяОбласти	= "СтатусВыполняется";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.ВПути Тогда
				ИмяОбласти	= "СтатусВПути";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.Выполнено Тогда
				ИмяОбласти	= "ДокументПроведен";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.Отменено Тогда
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.Отказ Тогда
				ИмяОбласти	= "СтатусОтказ";
			КонецЕсли;
		ИначеЕсли ТипЗначенияСсылки = Тип("ДокументСсылка.упРейс") Тогда	
			Если СтрокаДерева.Статус = Перечисления.упСтатусыРейса.Новый Тогда
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.КПланированиюТС Тогда
				ИмяОбласти	= "СтатусКПланированию";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.СформированыЗаявкиНаТС Тогда
				ИмяОбласти	= "СтатусСформированоЗадание";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.НазначеноТС Тогда
				ИмяОбласти	= "СтатусВключеноВРейс";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.КВыполнению Тогда
				ИмяОбласти	= "СтатусКВыполнению";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.Выполняется Тогда
				ИмяОбласти	= "СтатусВыполняется";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.Выполнен Тогда
				ИмяОбласти	= "ДокументПроведен";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.ВыполненЧастично Тогда
				ИмяОбласти	= "СтатусВыполненоЧастично";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.Отменен Тогда
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРейса.Завершен Тогда
				ИмяОбласти	= "СтатусЗавершен";
			КонецЕсли;
		ИначеЕсли ТипЗначенияСсылки = Тип("ДокументСсылка.упЗаявкаНаТС") Тогда		
			Если СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаТС.Создана Тогда
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаТС.Отправлена Тогда
				ИмяОбласти	= "СтатусОтправить";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаТС.ПолученоПредложение Тогда
				ИмяОбласти	= "СтатусПолученоПредложение";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаТС.Подтверждена Тогда
				ИмяОбласти	= "ДокументПроведен";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаТС.Отклонена Тогда
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			КонецЕсли;
		ИначеЕсли ТипЗначенияСсылки = Тип("ДокументСсылка.упПутевойЛист") Тогда		
			Если СтрокаДерева.Статус = Перечисления.упСтатусыПутевогоЛиста.Новый Тогда
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыПутевогоЛиста.Выдан Тогда
				ИмяОбласти	= "СтатусСформированоЗадание";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыПутевогоЛиста.Возвращен Тогда
				ИмяОбласти	= "СтатусКВыполнению";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыПутевогоЛиста.Закрыт Тогда
				ИмяОбласти	= "ДокументПроведен";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыПутевогоЛиста.Отменен Тогда
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			КонецЕсли;
		ИначеЕсли ТипЗначенияСсылки = Тип("ДокументСсылка.упВходящаяПретензия") Тогда		
			Если СтрокаДерева.Статус = Перечисления.упСтатусыВходящейПретензии.Новая Тогда
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыВходящейПретензии.ВРаботе Тогда
				ИмяОбласти	= "СтатусВыполняется";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыВходящейПретензии.Удовлетворена Тогда
				ИмяОбласти	= "ДокументПроведен";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыВходящейПретензии.УдовлетворенаЧастично Тогда
				ИмяОбласти	= "СтатусВыполненоЧастично";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыВходящейПретензии.Отменена Тогда
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыВходящейПретензии.Отклонена Тогда
				ИмяОбласти	= "СтатусОтказ";
			КонецЕсли;
		ИначеЕсли ТипЗначенияСсылки = Тип("ДокументСсылка.упИсходящаяПретензия") Тогда		
			Если СтрокаДерева.Статус = Перечисления.упСтатусыИсходящейПретензии.Новая Тогда
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыИсходящейПретензии.Отправлена Тогда
				ИмяОбласти	= "СтатусОтправить";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыИсходящейПретензии.Удовлетворена Тогда
				ИмяОбласти	= "ДокументПроведен";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыИсходящейПретензии.УдовлетворенаЧастично Тогда
				ИмяОбласти	= "СтатусВыполненоЧастично";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыИсходящейПретензии.Отменена Тогда
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыИсходящейПретензии.Отклонена Тогда
				ИмяОбласти	= "СтатусОтказ";
			КонецЕсли;
		ИначеЕсли ТипЗначенияСсылки = Тип("ДокументСсылка.упЗаявкаНаРемонтИТОТС") Тогда
			Если СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаРемонтИТОТС.Новая Тогда	
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаРемонтИТОТС.НеСогласована Тогда					
				ИмяОбласти	= "СтатусНеСогласовано";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаРемонтИТОТС.Согласована Тогда						
				ИмяОбласти	= "СтатусСогласовано";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаРемонтИТОТС.КВыполнению Тогда						
				ИмяОбласти	= "СтатусКВыполнению";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаРемонтИТОТС.СформированРемонт Тогда
				ИмяОбласти	= "СтатусСформированРемонт";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаРемонтИТОТС.ВРемонте Тогда						
				ИмяОбласти	= "СтатусВыполняется";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаРемонтИТОТС.Выполнена Тогда						
				ИмяОбласти	= "ДокументПроведен";			
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаявкиНаРемонтИТОТС.Отменена Тогда							
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			КонецЕсли;	
			
		ИначеЕсли ТипЗначенияСсылки = Тип("ДокументСсылка.упРемонтИТОТС") Тогда
			Если СтрокаДерева.Статус = Перечисления.упСтатусыРемонтаИТОТС.Новый Тогда
				ИмяОбласти	= "ДокументЗаписан";
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРемонтаИТОТС.Запланирован Тогда
				ИмяОбласти	= "СтатусСформированоЗадание";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРемонтаИТОТС.Начат Тогда
				ИмяОбласти	= "СтатусВыполняется";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРемонтаИТОТС.Завершен Тогда
				ИмяОбласти	= "ДокументПроведен";	
			ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыРемонтаИТОТС.Отменен Тогда
				ИмяОбласти	= "ДокументПомеченНаУдаление";
			КонецЕсли;
		КонецЕсли;		
	...
	Иначе	
		Если СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.Выполнено Тогда
			ИмяОбласти = "ДокументПроведен"
		ИначеЕсли СтрокаДерева.Статус = Перечисления.упСтатусыЗаданияНаПеревозкуГруза.Отменено Тогда
			ИмяОбласти = "ДокументПомеченНаУдаление"
		Иначе
	...
	Если ЭтоПодчиненный = Неопределено Тогда
		Если ДеревоПодчиненныеОбъекты.ПолучитьЭлементы().Количество() И ДеревоРодительскиеОбъекты.ПолучитьЭлементы().Количество()  Тогда
			ИмяОбласти = ИмяОбласти + "КоннекторВерхНиз";
		ИначеЕсли ДеревоПодчиненныеОбъекты.ПолучитьЭлементы().Количество() Тогда
			ИмяОбласти = ИмяОбласти + "КоннекторНиз";
		ИначеЕсли ДеревоРодительскиеОбъекты.ПолучитьЭлементы().Количество() Тогда
			ИмяОбласти = ИмяОбласти + "КоннекторВерх";
		Иначе
			ИмяОбласти = ИмяОбласти + "КоннекторВерх";
		КонецЕсли;
	ИначеЕсли ЭтоПодчиненный = Истина Тогда
		Если СтрокаДерева.ПолучитьЭлементы().Количество() Тогда 
			ИмяОбласти = ИмяОбласти + "КоннекторЛевоНиз";
		КонецЕсли;
	Иначе
		Если СтрокаДерева.ПолучитьЭлементы().Количество() Тогда 
			ИмяОбласти = ИмяОбласти + "КоннекторЛевоВерх";
		КонецЕсли;
	КонецЕсли;
	ОбластьКартинка = Макет.ПолучитьОбласть(ИмяОбласти);
	// ---.17/01/16-16:20:56.<УП>
...
Функция ПолучитьВыборкуПоРеквизитамОбъекта(ОбъектСсылка)
	...
	//|";
	//
	ТипЗначенияСсылки	= ТипЗнч(ОбъектСсылка);
	СтатусНовый			= Неопределено;
	СтатусОтменено		= Неопределено;
	Если НЕ ТипыСоСтатусами.НайтиПоЗначению(ТипЗначенияСсылки) = Неопределено Тогда
		ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	тбДокумент.Ссылка,
			|	ВЫБОР
			|		КОГДА ПометкаУдаления
			|			ТОГДА &СтатусОтменено
			|		ИНАЧЕ ЕСТЬNULL(тбСтатусы.Статус, &СтатусНовый)
			|	КОНЕЦ КАК Статус,
			|	#Проведен,
			|	ПометкаУдаления,
			|	#Сумма,
			...
			|	#Представление
			|ИЗ
			|	" + МетаданныеОбъекта.ПолноеИмя() + " КАК тбДокумент 
			| ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений." + упРаботаСоСтатусами.ПолучитьИмяРегистра(ОбъектСсылка) + ".СрезПоследних(, Документ = &Ссылка) КАК тбСтатусы
			|ПО тбДокумент.Ссылка = тбСтатусы.Документ
			|ГДЕ тбДокумент.Ссылка = &Ссылка";
		СтатусНовый		= упРаботаСоСтатусами.ПолучитьСтатусНовый(ОбъектСсылка);
		СтатусОтменено 	= упРаботаСоСтатусами.ПолучитьСтатусОтменено(ОбъектСсылка);
	Иначе
		ТекстЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	Ссылка,
		...
		|";
	КонецЕсли;	
	ЗаменитьТекстЗапроса(ТекстЗапроса, МетаданныеОбъекта, "#Сумма", ИмяРеквизитаДокумента(МетаданныеОбъекта.Имя, "СуммаДокумента"), "СуммаДокумента");
	ЗаменитьТекстЗапроса(ТекстЗапроса, МетаданныеОбъекта, "#Валюта", ИмяРеквизитаДокумента(МетаданныеОбъекта.Имя, "Валюта"), "Валюта");
	// ---.16/11/29-12:05:55.<УП>
	...
	// +++.16/11/29-12:14:42.<УП>
	Если НЕ СтатусНовый = Неопределено Тогда
		Запрос.УстановитьПараметр("СтатусНовый", СтатусНовый);
	КонецЕсли;
	Если НЕ СтатусОтменено = Неопределено Тогда
		Запрос.УстановитьПараметр("СтатусОтменено", СтатусОтменено);
	КонецЕсли;
	// ---.16/11/29-12:14:42.<УП>
...
Процедура ВывестиПодчиненныеОбъекты(ТекущийОбъект,ДеревоРодитель)
	...
	Для Каждого КлючИЗначение Из КэшПоТипамОбъектов Цикл
		...
		//|	Ссылка В (&" + ИмяОбъекта + ")";
		//
		СсылкаПодчиненного = КлючИЗначение.Значение[0];
		ТипЗначенияСсылки = ТипЗнч(СсылкаПодчиненного);
		ИндексТипаСсылки = ТипыСоСтатусами.НайтиПоЗначению(ТипЗначенияСсылки);
		Если ИндексТипаСсылки <> Неопределено Тогда
			ИмяРегистраСтатусов = упРаботаСоСтатусами.ПолучитьИмяРегистра(СсылкаПодчиненного);
			ИмяПараметраНовый = "СтатусНовый" + ИмяРегистраСтатусов;
			ИмяПараметраОтменено = "СтатусОтменено" + ИмяРегистраСтатусов;
			ТекстПоТипуОбъекта = "
			|	ВЫБОР
			|		КОГДА ПометкаУдаления
			|			ТОГДА &" + ИмяПараметраОтменено + "
			|		ИНАЧЕ ЕСТЬNULL(тбСтатусы.Статус, &" + ИмяПараметраНовый + ")
			|	КОНЕЦ КАК Статус,
			|" + КэшРеквизитовОбъекта[КлючИЗначение.Ключ]["Дата"] + "           КАК Дата,
			|	Ссылка,
			|" + КэшРеквизитовОбъекта[КлючИЗначение.Ключ]["Проведен"] + "       КАК Проведен,
			...
			|	#Представление
			|ИЗ
			|	" + КлючИЗначение.Ключ +  " КАК тбДокумент ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений." + ИмяРегистраСтатусов + ".СрезПоследних(, Документ В (&" + ИмяОбъекта + ")) КАК тбСтатусы
			|ПО тбДокумент.Ссылка = тбСтатусы.Документ
			|ГДЕ
			|	тбДокумент.Ссылка В (&" + ИмяОбъекта + ")";
			
			СтатусНовый		= упРаботаСоСтатусами.ПолучитьСтатусНовый(СсылкаПодчиненного);
			СтатусОтменено 	= упРаботаСоСтатусами.ПолучитьСтатусОтменено(СсылкаПодчиненного);
			
			Запрос.УстановитьПараметр(ИмяПараметраНовый, СтатусНовый);
			Запрос.УстановитьПараметр(ИмяПараметраОтменено, СтатусОтменено);
		Иначе
			ТекстПоТипуОбъекта = "
			|	ВЫБОР
			|		КОГДА Проведен
...
Функция ДобавитьСтрокуВДерево(СтрокиДерева, Выборка)
	...
	// @@@.16/11/29-12:18:13.<УП>
	ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка, "Статус");
