...
Процедура ПриСозданииПриЧтенииНаСервере()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		Объект.ВидТранспортнойУслуги	 = Перечисления.упВидыТранспортныхУслуг.ПеревозкаПассажиров;
	КонецЕсли;
...
&НаКлиенте
Процедура МаршрутСледованияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если Ложь
		Или ЗначениеЗаполнено(Объект.АдресОтправления) 
		Или ЗначениеЗаполнено(Объект.АдресПолучения) 
	Тогда 
		СтандартнаяОбработка = Ложь;
		мсвМаршруты = МаршрутыПоАдресамОтправленияПолучения(Объект.АдресОтправления, Объект.АдресПолучения);
		сткОтбор = Новый Структура("Ссылка", мсвМаршруты);
		сткПараметры = Новый Структура("Отбор", сткОтбор);
		ОткрытьФорму("Справочник.упТиповыеМаршруты.ФормаВыбора", сткПараметры, Элемент, УникальныйИдентификатор);
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Функция МаршрутыПоАдресамОтправленияПолучения(АдресОтправления, АдресПолучения)
	Возврат Справочники.упТиповыеМаршруты.МаршрутыПоАдресамОтправленияПолучения(АдресОтправления, АдресПолучения);
КонецФункции
