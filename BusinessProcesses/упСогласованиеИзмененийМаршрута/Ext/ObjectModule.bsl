...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если Истина
		...
		И (Ложь
		   Или ДанныеЗаполнения.ТипСобытия = Справочники.упТипыСобытийМониторинга.ЗапросНаДосрочноеУбытиеИзТочкиМаршрута
		   Или ДанныеЗаполнения.ТипСобытия = Справочники.упТипыСобытийМониторинга.ЗапросНаДосрочноеЗавершениеРейса
		   Или ДанныеЗаполнения.ТипСобытия = Справочники.упТипыСобытийМониторинга.ЗапросНаНазначениеТочкиСледующейПоМаршруту)
	Тогда
		
		Предмет = ДанныеЗаполнения;
		сткДанныеЗаполнения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, "Рейс, РезультатОбработки, СтатусЗапроса, АдресТочки");
		СтатусЗапроса = сткДанныеЗаполнения.СтатусЗапроса;
		РезультатВыполнения = сткДанныеЗаполнения.РезультатОбработки;

		Если ДанныеЗаполнения.ТипСобытия = Справочники.упТипыСобытийМониторинга.ЗапросНаДосрочноеУбытиеИзТочкиМаршрута Тогда
			Наименование = СтрШаблон(Нстр("ru = 'Согласование досрочного убытия из точки по адресу ""%1"" для %2'"), сткДанныеЗаполнения.АдресТочки, сткДанныеЗаполнения.Рейс);
		ИначеЕсли ДанныеЗаполнения.ТипСобытия = Справочники.упТипыСобытийМониторинга.ЗапросНаДосрочноеЗавершениеРейса Тогда
			Наименование = СтрШаблон(Нстр("ru = 'Согласование досрочного завершения рейса в точке ""%1"" для %2'"), сткДанныеЗаполнения.АдресТочки, сткДанныеЗаполнения.Рейс);
		ИначеЕсли ДанныеЗаполнения.ТипСобытия = Справочники.упТипыСобытийМониторинга.ЗапросНаНазначениеТочкиСледующейПоМаршруту Тогда	
			Наименование = СтрШаблон(Нстр("ru = 'Согласование назначения следующей точкой ""%1"" маршрута для %2'"), сткДанныеЗаполнения.АдресТочки, сткДанныеЗаполнения.Рейс);
		КонецЕсли;
...
Процедура ВыполнитьПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	...
	Если Не Перенаправление Тогда
		БизнесПроцессы.упСогласованиеИзмененийМаршрута.ОбработкаПроверкиЗаполненияЗадачи(Задача, Отказ);
	КонецЕсли;

	Если Не Отказ Тогда
		СтатусЗапроса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Задача.Предмет, "СтатусЗапроса");
