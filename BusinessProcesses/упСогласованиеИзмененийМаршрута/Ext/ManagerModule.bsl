...
Функция ФормаВыполненияЗадачи(ЗадачаСсылка, ТочкаМаршрутаБизнесПроцесса) Экспорт
	...
	Результат.Вставить("ИмяФормы", "БизнесПроцесс.упСогласованиеИзмененийМаршрута.Форма.Действие" + ТочкаМаршрутаБизнесПроцесса.Имя);
	Возврат Результат;
...
Процедура ОбработкаПроверкиЗаполненияЗадачи(Задача, Отказ) Экспорт
	...
	Если Задача.Выполнена Тогда
		...
		Если Не ЗначениеЗаполнено(СтатусЗапроса)  Тогда
			ТекстСообщения = Нстр("ru = 'Не заполнен статус запроса.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"СтатусЗапроса",,Отказ);
		КонецЕсли;
	КонецЕсли;
