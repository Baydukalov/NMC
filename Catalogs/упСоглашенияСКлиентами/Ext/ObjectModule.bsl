...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	тбзСтатьиДоходов = СтатьиДоходов.Выгрузить();
	тбзСтатьиДоходов.Свернуть("СтатьяДоходов");
	
	Если НЕ тбзСтатьиДоходов.Количество() = СтатьиДоходов.Количество() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("ru = 'Список статей доходов содержит повторяющиеся статьи.'");
		Отказ = Истина;
	КонецЕсли;
