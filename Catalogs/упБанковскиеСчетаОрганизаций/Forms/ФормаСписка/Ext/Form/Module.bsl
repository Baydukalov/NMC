...
Процедура ИспользоватьКакОсновной(Команда)
	...
	Если НЕ ТекущиеДанные = Неопределено Тогда
		...
		Если ТекущиеДанные.Основной Тогда
			ТекущийСчет = ПредопределенноеЗначение("Справочник.упБанковскиеСчетаОрганизаций.ПустаяСсылка");
		Иначе	
...
Процедура УстановитьСчетПоУмолчанию(Владелец, Счет, Отказ)
	...
	Исключение
			...
			КонецЕсли;
			ТекстСообщения = Нстр("ru = 'Перед изменением основного счета необходимо записать организацию.'");
