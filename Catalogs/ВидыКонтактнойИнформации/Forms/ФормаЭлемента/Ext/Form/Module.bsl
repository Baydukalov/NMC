...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// @@@.16/11/29-10:45:35.<УП>
	упРольДоступнаупРазработчик = РольДоступна("упРазработчик");
	Если Истина
		И (Ложь
			Или Объект.Предопределенный 
			Или Объект.ЗапретитьРедактированиеПользователем)
		// @@@.16/11/29-10:44:59.<УП>
		И Не упРольДоступнаупРазработчик
	Тогда
...
//}}MRG[ <-> ]
//{{MRG[ <-> ]
//	Элементы.ХранитсяИсториюИзменений.Видимость = Объект.ХранитьИсториюИзменений;
//}}MRG[ <-> ]
//{{MRG[ <-> ]
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Ложь
		Или Не ТекущийОбъект.Предопределенный 
		// @@@.16/11/29-10:47:45.<УП>
		Или (ТекущийОбъект.Предопределенный И упРольДоступнаупРазработчик)
	Тогда
		// Обработчик подсистемы запрета редактирования реквизитов объектов.
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
			МодульЗапретРедактированияРеквизитовОбъектов = ОбщегоНазначения.ОбщийМодуль("ЗапретРедактированияРеквизитовОбъектов");
...
Процедура Подключаемый_РазрешитьРедактированиеРеквизитовОбъекта(Команда)
	...
	Если Ложь
		...
		// @@@.16/11/29-10:46:28.<УП>
		Или (Объект.Предопределенный И упРольДоступнаупРазработчик)
	Тогда
